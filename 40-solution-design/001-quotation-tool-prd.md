...
3. ~~Excel-to-table conversion~~ **Resolved.** See `40-solution-design/002-change-note-production-table.md`. Sample Excel received (`Production Chart Working_P512.xlsx`). Production table has 11 columns, variable rows per bar size. Design Parameters (8 fields) and summary statistics parsed from the same Excel. `production_table_data` JSON structure defined in the change note.
